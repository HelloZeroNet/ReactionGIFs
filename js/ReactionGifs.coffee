class ZeroBlog extends ZeroFrame
	...
	checkLazyVideos: =>
		...
		if scroll_y == 0  # Always play the first video on top scroll
			center = 0
		else
			center = scroll_y + inner_h/2
		closest_video = null
		closest_distance = 1000
		# Only play one video that is closest to center
		for lazy in @lazy_videos
			[video, video_top, video_height] = lazy
	...
	loadLastcomments: (type="show", cb=false) ->
		return false  # No need on reactiongifs
		query = "
	...
	setSiteinfo: (site_info) =>
		...
		if site_info.event?[0] == "file_done" and site_info.event[1].match /.*users.*data.json$/
			...
			@loadData()
			if $("body").hasClass("page-main") then @pageMain("update")
